...
<TR>
	<TD ALIGN=CENTER>
		...
		if (C_DB_TYPE == "mysql" || C_DB_TYPE == "mysqli" || C_DB_TYPE == "mariadb") $limits = " LIMIT $startReg,10";
		elseif (C_DB_TYPE == "pgsql") $limits = " LIMIT 10 OFFSET $startReg";
		else $limits = "";

		$DbLink->query("SELECT username,latin1,perms,rooms,reg_time,colorname,last_login,ip,login_counter,join_room,country_code,country_name FROM ".C_REG_TBL." WHERE email != 'bot@bot.com' AND email != 'quote@quote.com' AND username != '".$pmc_username."' ORDER BY ".($sortBy != "ip" ? $sortBy : "country_code OR ".$sortBy)." ".$sortOrder.$limits);
		$bannished_user = "";
		$bannished_ip = "";
		$DbLinkban = new DB;
		...
		while (list($username,$Latin1,$perms,$rooms,$regTime,$colorName,$lastTime,$IP,$login_counter,$join_room,$COUNTRY_CODE,$COUNTRY_NAME) = $DbLink->next_record())
		{
			// GeoIP mode for country flags
			if(C_USE_FLAGS)
			{
...
#			$IP_table = "<td>{$nums[0]}.</td><td>{$nums[1]}.</td><td>{$nums[2]}.</td><td>{$nums[3]}</td>";
			...
			<TR>
				...
				<TD VALIGN=CENTER ALIGN="<?php echo($CellAlign); ?>">
					<B><FONT color=<?php echo("\"$colorName\">".special_char($username,$Latin1)."".$bannished_user);?></FONT></B>
				</TD>
				<TD VALIGN=CENTER ALIGN=CENTER>
			...
			<?php
			unset($username,$Latin1,$perms,$rooms,$regTime,$colorName,$lastTime,$IP,$login_counter,$join_room,$COUNTRY_CODE,$COUNTRY_NAME,$bannished_user,$bannished_ip);
