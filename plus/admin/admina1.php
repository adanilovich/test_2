...
<?php
	...
	$DbLink->query("SELECT username,firstname,lastname,gender,avatar,use_gravatar,email,showemail,birthday,show_bday,show_age,ip,colorname,country_code,country_name,((YEAR(CURDATE())-YEAR(birthday)) - (RIGHT(CURDATE(),5)<RIGHT(birthday,5))) AS age FROM ".C_REG_TBL." WHERE ".$CondForQuery."".$sqlT."".$limit."");
?>
...
<FORM ACTION="<?php echo($pstr."&mord=".$mord."&sortOrder=".$sortOrder.($cYr ? "&cYr=".$cYr : "").($use_limT ? "&limT=".$limT : "")); ?>" METHOD="POST" AUTOCOMPLETE="OFF" NAME="FORM1" onSubmit="reload();">
	...
	<?php
		...
		while(list($User,$dob_firstname,$dob_lastname,$gender,$avatar,$use_gravatar,$email,$dob_showemail,$dob_birthday,$dob_showbday,$dob_showage,$IP,$colorName,$COUNTRY_CODE,$COUNTRY_NAME,$dob_age) = $DbLink->next_record())
		{
...
#						if (eregi(C_AVA_RELPATH, $avatar)) $local_avatar = 1;
				...
				if ($User != $pmc_username) $Edit = "<a onClick=\"browse_user($dob_username);\" target=\"_self\" title=\"".A_SEARCH_25."\"><img src=images/edit.gif alt='".A_SEARCH_25."' title='".A_SEARCH_25."'></a>";
				?>
				<?php
				...
				echo("<TD style=\"vertical-align:middle; text-align:left;\"><B><FONT color=\"".$colorName."\">".$User."</FONT></B>".$Edit."</TD>");
				// GeoIP mode for country flags
				if(C_USE_FLAGS)
				{
			...
			unset($User,$dob_firstname,$dob_lastname,$gender,$avatar,$use_gravatar,$email,$dob_showemail,$dob_birthday,$dob_showbday,$dob_showage,$IP,$colorName,$COUNTRY_CODE,$COUNTRY_NAME,$dob_age,$dob_name,$dobtime,$dob_time,$gender1,$ava_width,$Edit);
		}
		// GeoIP mode for country flags
...
&copy; 2012-<?php echo(date('Y')); ?> - by <a href="mailto:<?php echo(PLUS_DEVELOPER_EMAIL); ?>?subject=phpMychat%20Plus%20feedback" onMouseOver="window.status='<?php echo(sprintf(L_CLICKS,L_LINKS_6,L_AUTHOR)); ?>.'; return true;" title="<?php echo(sprintf(L_CLICKS,L_LINKS_6,L_AUTHOR)); ?>" target=_blank><?php echo(PLUS_DEVELOPER); ?></a></span></div></P>
<?php
?>