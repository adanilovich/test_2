...
if (isset($MailFunctionOn))
{
	...
	function send_email_admin($To,$Subject,$Body,$BCC)
	{
		global $Charset;
...
#		$Headers = "From: ${Sender_Name} <${Sender_email}>".$eol;
		if (${pmc_email}) $Headers = "Reply-To: ${pmc_email}".$eol;
		else $Headers = "Reply-To: ${Sender_email}".$eol;
...
/*		if (!stristr(PHP_OS,'win')) {
	...
	#		$Headers .= function_exists("base64_encode") ? wordwrap(base64_encode(strip_tags($Body)), 70, $eol) : wordwrap(strip_tags($Body), 70, $eol);
	...
	#		$Headers .= chunk_split(str_replace($eol,"<br />",$Body));
			$Headers .= "Content-Transfer-Encoding: ".(function_exists("base64_encode") ? "base64" : "8bit").$eol.$eol;
			$Body = function_exists("base64_encode") ? wordwrap(base64_encode(strip_tags($Body)), 70, $eol) : wordwrap(strip_tags($Body), 70, $eol);
	#		$Headers .= function_exists("base64_encode") ? wordwrap(base64_encode(str_replace($eol,"<br />",$Body)), 70, $eol) : wordwrap(str_replace($eol,"<br />",$Body), 70, $eol);
...
*/
			...
			$Body = wordwrap(strip_tags($Body), 70, $eol);
