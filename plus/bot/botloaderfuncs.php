...
global $selectbot, $annesID, $cdbtype, $conn;
...
function empty_tables()
{
	...
	if($cdbtype == "mysqli") $e1 = mysqli_query($conn, $q1);
	else $e1 = mysql_query($q1);
	if($e1){
    }
	$q2="TRUNCATE TABLE bot_bots";
	if($cdbtype == "mysqli") $e2 = mysqli_query($conn, $q2);
    else $e2 = mysql_query($q2);
    if($e2){
    }
	$q3="TRUNCATE TABLE bot_patterns";
	if($cdbtype == "mysqli") $e3 = mysqli_query($conn, $q3);
    else $e3 = mysql_query($q3);
    if($e3){
    }
	$q4="TRUNCATE TABLE bot_templates";
	if($cdbtype == "mysqli") $e4 = mysqli_query($conn, $q4);
    else $e4 = mysql_query($q4);
    if($e4){
    }
	$q5="TRUNCATE TABLE bot_thatindex";
	if($cdbtype == "mysqli") $e5 = mysqli_query($conn, $q5);
    else $e5 = mysql_query($q5);
    if($e5){
    }
	$q6="TRUNCATE TABLE bot_thatstack";
	if($cdbtype == "mysqli") $e6 = mysqli_query($conn, $q6);
    else $e6 = mysql_query($q6);
    if($e6){
    }
}
...
function deletebot($bot)
{
	...
	if($cdbtype == "mysqli") $e = mysqli_query($conn, $q);
    else $e = mysql_query($q);
    if($e){
    }
...
function findwordid($word,$parent)
{
	...
	if($cdbtype == "mysqli"){
        $selectcode = mysqli_query($conn, $query);
        if($selectcode){
...
function setnotend($wordid)
{
    ...
    if($cdbtype == "mysqli") $q=mysqli_query($conn, $query);
	else $q=mysql_query($query);
    if($q){
    }
}
...
{
	global $patternsinserted, $cdbtype, $conn;

    if($cdbtype == "mysqli") $qcode=mysqli_query($conn, "insert into bot_patterns(bot,id,word,ordera,parent,isend) values $qadd");
	else $qcode=mysql_query("insert into bot_patterns(bot,id,word,ordera,parent,isend) values $qadd");
    $patternsinserted++;

	if($qcode){

		if($cdbtype == "mysqli") return ((is_null($___mysqli_res = mysqli_insert_id($conn))) ? false : $___mysqli_res);
...
{
	global $selectbot, $templatesinserted, $pattern, $topic, $that, $cdbtype, $conn;
    ...
    if(!templateexists($idused)){
        ...
        if($cdbtype == "mysqli") $qcode=mysqli_query($conn, $query);
		else $qcode=mysql_query($query);
        if($qcode){
        }
    }
}
...
{
    global $selectbot, $whaton, $startupwhich, $splitterarray, $inputarray, $genderarray, $personarray, $person2array, $allbots, $areinc, $cdbtype, $conn;
    ...
    if(strtoupper($name)=="PROPERTY"){
        ...
        if($cdbtype == "mysqli") $qcode=mysqli_query($conn, $q);
		else $qcode=mysql_query($q);
        if($qcode){
        }
    ...
    elseif(strtoupper($name)=="BOT"){
		...
		if($areinc==1){
			...
			if($cdbtype == "mysqli") $newbotid=((is_null($___mysqli_res = mysqli_insert_id($conn))) ? false : $___mysqli_res);
			else $newbotid=mysql_insert_id();
		}
		else{
			$newbotid=$existbotid;
		}
...
function botexists($name)
{
    ...
    if($cdbtype == "mysqli"){
        $selectcode = mysqli_query($conn, $q);
        if($selectcode){
            while($q = mysqli_fetch_array($selectcode)){
                return true;
            }
...
#        $myphp.="\"/$beginsearch" . trim($searchvar) . "$endsearch/ie\",\n";
        $myphp.="\"/$beginsearch" . trim($searchvar) . "$endsearch/i\",\n"; // \ie is depracated on php7
    }
