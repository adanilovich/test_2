...
function cleanup(){
	...
	if($randval==RANDOMCHANCECLEAN){
		...
		if(MINUTESTOKEEPDATA != -1){
			...
			if($cdbtype == "mysqli"){
				$selectcode = mysqli_query($conn, $clean_dstore);
				if($selectcode){
				}
				$selectcode = mysqli_query($conn, $clean_thatstack);
				if($selectcode){
				}
				$selectcode = mysqli_query($conn, $clean_thatindex);
				if($selectcode){
				}
			}
			else{
        ...
        if(MINUTESTOKEEPCHATLOG != -1){
			...
			if($cdbtype == "mysqli") $selectcode = mysqli_query($conn, $clean_convlog);
			else $selectcode = mysql_query($clean_convlog);
			if($selectcode){
			}
...
#	return stristr(PHP_OS,'win') ? utf_conv(WIN_DEFAULT,$Charset,strftime($date_format)) : strftime($date_format);
	...
	setlocale(LC_TIME, "en_US.UTF-8", "enu.UTF-8", "usa.UTF-8", "enu_enu.UTF-8", "English-usa.UTF-8"); // For American formats
	}
...
function botget($name){
	...
	global $uid, $selectbot, $cdbtype, $conn;
...
function bset($name,$value){
    ...
    if($cdbtype == "mysqli") $selectcode = mysqli_query($conn, $query);
	else $selectcode = mysql_query($query);
	if($selectcode){
	}
}
...
function addthats($inputsarray){
	...
	if($cdbtype == "mysqli"){
		...
		$thatidx=((is_null($___mysqli_res = mysqli_insert_id($conn))) ? false : $___mysqli_res);
	}
	else{
		$selectcode = mysql_query($query);
		if($selectcode){
