...
Client browsers - tested with (both 32b & 64b where apply):
	- IE < 5.5 = M;
	- IE > 6.0SP2 (including > 7.0 & > 8.0 & > 9.0 & > 10.0 & > 11.0) = H;
	- Mozilla Aurora = 12.0 = H;
	- Mozilla Firefox > 1.6.0.0 (up to and including 57 on 64b) = H;
	...
	- Google Chrome > 62.x.x.x.x = H;
	- Microsoft Edge;
	- Tor Browser < 6.0.8 = H.
Server environment - tested env:
	- Apache < 2.4.29 (Unix and Windows 2k/XP/Vista/7/8/8.1/10);
	- ISS < 7;
	- php < 7.1.11;
	- MySQL < 5.7.20; (mysqli extension recommended, but mysql extension is still available for some old versions)
	- MariaDB 5.5.54 & 10.2.10;
...
Note: development has been carried out on WampServer 3.1.0 64b, running on Windows 10 Home Insider Preview 64b & Windows 8.1 64b;

Known issues (read also the FAQ):
- in Firefox Quantum (>57) there is no support for notification sounds (embeded audio files);
...
Fixes History:
13.11.2017 - 1.96
- updated the Dutch statistics translation file (functionality related);
- fixed the blank page that was throwing warnings (functionality related);
- fixed a log format bug introduced with this new release (functionality related);
- fixed a null selection bug on the calendar (functionality related);
- fixed a layout bug on the calendar info page (functionality related);
- fixed the buzz_popup page to correctly build the table (funtionality related);
- logs display improved (functionality related);
- bot has been improved to deal with punctuation marks and single quotes (functionality related);
- phpMyChat-Plus is now working on both SSL (https) and MySQL/MariaDB safe mode on (functionality related);
- installer/updater process improved (functionality related);
- improved sound notifications for entrances (functionality related);
- added Cokie Consent policy pop-up information to comply with EU/national cookie laws (functionality related);
- sending mail has been fixed; the new Sender Policy Framework (SPF) was introduced to prevent email source address spoofing and many ISPs implemented this protection, so the From: field had to be removed from email headers (functionality/security related)
- GeoIP databases updated to latest version from maxmind.com (functionality related);
- IE11 recognized like H browser (mozilla/gecko) (functionality related);
- added new feature: sound notification on new event in chat window (new chat message, user entrance, etc.) - it uses cookies (functionality related);
- removed option in profile for users to enable/disable sound notifications in chat (it was introduced in 1.94-RC5 but we dropped it as the cookiebeep above is enough); (functionality extension)
- changed the 5 ip fields in the database to varchar(100) to allow IPv6 connections to the chat (Important: this is a bug fix for users being kicked out on login without any error!); (functionality related)
- added IPv6 support (functionality related);
01.01.2017 - 1.95
- fixed a calendar bug when accessing from a secure protocol (functionality related);
- fixed the check_internet_connection() script (functionality related);
- config.lib and install files optimized (functionality related);
- youtube class updated (functionality related);
- fixed current logs folder permissions to allow reading logs (functionality related);
- Google Chrome is now recognized as H (previously it only worked as M) thus flickering effect in chat has now been fixed (functionality related);
- media player updated to html5 video tag with HLS support (functionality related);
- automatic installer fixes (functionality related);
- code improvements and fixes (functionality related);
- php5 to php7 full compatibility (functionality related);
- added full MariaDB support (functionality extension);
- added full MySQLi support for php>5.0.0 (functionality related);
- fixed a display issue of the log files on php5 servers (functionality related);
01.01.2015 - 1.94-RC5
- calendar class fix for php7 and backward compatibility (functionality related);
- GeoIP databases and geoip.inc from maxmind.com updated (regular update);
- Finnish language added (localization project) - Thanks to Mirja-Liisa Väänänen, ShadowKiro <shadowkiro@gmail.com>.
- colornames added in admin tabs (regusers, profiles and search); (functionality related)
- calendar class updated to 3.75-loc version (fixed missing year on selection); (functionality related)
- added option in profile for users to enable/disable sound notifications in chat (it doesn't apply to buzzes and media posts e.g. youtube); (functionality extension)
...
- changed the 4 ip fields in the database to varchar(30) to allow IPv6 connections to the chat (Important: this is a bug fix for users being kicked out on login without any error!); (functionality related)
