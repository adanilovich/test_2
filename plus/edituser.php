...
#	else if (!preg_match("/^([0-9a-z]([-_.]?[0-9a-z])*@[0-9a-z]([-.]?[0-9a-z])*\\.[a-wyz][a-z](avel|bi|bs|fo|g|ia|l|m|me|mes|o|op|pa|ro|seum|t|to|u|v|z)?)$/i", $EMAIL))
	...
	else if(C_REQUIRE_BDAY && (!$BIRTHDAY || strpos($BIRTHDAY, "00") === 0 || strpos($BIRTHDAY, "-00") > 0))
	{
		$Error = L_ERR_USR_29;
	}
...
#		$Headers = "From: ${Sender_Name} <${Sender_email}>".$eol;
		...
		if (C_EMAIL_USER || (C_EMAIL_PASWD && !C_EMAIL_USER))
		{
	     ...
	     . $Mail_Greeting.$eol.$Sender_Name1.$eol.$Chat_URL;
			...
			$emailMessage = wordwrap(stripslashes($emailMessage), 70, $eol);
...
<link href="plugins/calendar/calendar.css" rel="stylesheet" type="text/css" />
<?php
...
#				$myCalendar->setTimezone("Europe/Bucharest");
			...
			$DbLink->close();
			if (!$done && C_REQUIRE_BDAY) echo("&nbsp;<SPAN CLASS=\"error\">*</SPAN>"); ?>
