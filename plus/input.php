...
if (file_exists("./localization/${L}/localized.config.php")) require("./localization/${L}/localized.config.php");
else require("./localization/english/localized.config.php");
...
// Bob Dickow Custom code for /away command modification - modified by Ciprian for Plus behaviour.:
   ...
   if ($awaystat == 1) {
	...
	if(C_EN_STATS)
	{
		$DbLink->query("UPDATE ".C_STS_TBL." SET seconds_away=seconds_away+(".time()."-last_away), longest_away=IF(".time()."-last_away < longest_away, longest_away, ".time()."-last_away), last_away='0' WHERE (stat_date=FROM_UNIXTIME(last_away,'%Y-%m-%d') OR stat_date=FROM_UNIXTIME(last_in,'%Y-%m-%d')) AND room='$R' AND username='$U'");
	}
   }
...
function swapImage(img,imgid) {
	...
	if (imgid == "beepToSwap")
	{
		var path = '<?php echo("./images/"); ?>';
		var beep = "sound.gif";
		if (image.checked) {
			image.checked = '';
			beep = "nosound.gif";
			setCookie("CookieBeep",0,0)
		} else {
			image.checked = 'checked';
			beep = "sound.gif";
			setCookie("CookieBeep",1,365)
		}
		image.src = path + beep;
	}
};
...
{
?>
	<A HREF="<?php echo("buzz_popup.php?L=$L"); ?>" onClick="window.parent.buzz_popup(); return false" CLASS="ChatReg" onClick="document.forms['MsgForm'].elements['M'].focus();" onMouseOver="window.status='<?php echo(L_BUZZ); ?>.'; return true" TARGET="_blank" title="<?php echo(L_BUZZ); ?>"><img src="images/buzz.gif" border=0 alt="<?php echo(L_BUZZ); ?>"></A>
<?php
}
?>
...
// Display the server time in the clock text box
	include_once("./lib/worldtime.lib.php");
...
// -->
	...
	&nbsp;<img style="vertical-align:middle; cursor:pointer; cursor:hand;" id="beepToSwap" onClick="swapImage('beep','beepToSwap')" src="<?php echo("./images/".(isset($CookieBeep) && $CookieBeep ? "sound.gif" : "nosound.gif").""); ?>" BORDER=0 <?php echo("ALT=\"".L_REG_53."\" Title=\"".L_REG_53."\""); ?> />
</TD>
</TR>
</FORM>
</TABLE>
