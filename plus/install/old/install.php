...
// Added for php5 support of mysqli library
$cdbtype = function_exists('mysqli_connect') ? "mysqli" : "mysql";
...
if ( $_SESSION['dbport'] != "" ) $dbport = $_SESSION['dbport'];
if ( $_POST['dbport'] != "" ) $dbport = $_POST['dbport'];
...
if ( $_SESSION['dbdriver'] != "" ) $dbdriver = $_SESSION['dbdriver'];
if ( $_POST['dbdriver'] != "" ) $dbdriver = $_POST['dbdriver'];
$_SESSION['dbdriver'] = $dbdriver;
if ( $_SESSION['dbprefix'] != "" ) $dbprefix = $_SESSION['dbprefix'];
if ( $_POST['dbprefix'] != "" ) $dbprefix = $_POST['dbprefix'];
...
##					if (is_dir($ChatPath.$logdirnew)) { if (ftp_chmod($conn_id, $mode7, $ftppath.$logdirnew) !== false) {} else { $error3 .= L_FOLD_ERROR1." &quot;/".$logdirnew."&quot; ".L_FOLD_ERROR2."<br /><br />\n"; } }
	...
	{
		include ( $ChatPath."lib/release.lib.php" );
		if(file_exists( $ChatPath."config/curr_version.lib.php" )) include( $ChatPath."config/curr_version.lib.php" );
		else
		{
			if ( ! $fhc = fopen ( $ChatPath."config/curr_version.lib.php", "w" ) )
			{
				$error3 = L_P3_HINT5;
			}	// END OF COULD NOT OPEN
			else {
			  $lfeed = "\r\n";
			  if ( ! fputs ( $fhc, '<?php'.$lfeed ) )
			  {
					$error3 = 'Could not write into "config/curr_version.lib.php"!';
			  }	// END OF COULD NOT WRITE
			  else {
					fputs ( $fhc, '// App current version values for future upgrade purposes (it shouldn’t be empty after the installation process)'.$lfeed );
					fputs ( $fhc, 'define("APP_CURR_VERSION", \''.APP_VERSION.'\');'.$lfeed );
					fputs ( $fhc, 'define("APP_CURR_MINOR", \''.APP_MINOR.'\');	// E.g. "-ß1"'.$lfeed );
					fputs ( $fhc, '?>' );
				}	// END OF WRITE INTO config/curr_version.lib.php
				fclose ( $fhc );
			}	// END OF OPEN THE FILE
			define("APP_CURR_VERSION", APP_VERSION);
			define("APP_CURR_MINOR", APP_MINOR);
		}
		if (APP_CURR_VERSION != "")
		{
			if (preg_match("/$(v1.[0-6])/i",APP_CURR_VERSION)) $kind == "1016";
			elseif (preg_match("/^(0.1[4-5](.[0-9])?)/",APP_CURR_VERSION)) $kind == "014015";
			elseif (preg_match("/^(0.1[2-3](.[0-9])?)/",APP_CURR_VERSION)) $kind == "013";
			elseif (preg_match("/^(0.1[1-2](.[0-9])?)/",APP_CURR_VERSION)) $kind == "012";
			else
			{
				switch (APP_CURR_VERSION)
				{
					case "1.96":
					{
						if (APP_CURR_MINOR != "" && strstr(APP_CURR_MINOR,"-ß1")) $kind = "195";
					}
					break;
					case "1.95":
					{
					...
					case "1.94":
					{
						if (APP_CURR_MINOR != "" && strstr(APP_CURR_MINOR,"RC5")) $kind = "194-RC5";
						elseif (APP_CURR_MINOR != "" && strstr(APP_CURR_MINOR,"RC4")) $kind = "194-RC4";
						elseif (APP_CURR_MINOR != "" && strstr(APP_CURR_MINOR,"RC3")) $kind = "194-RC3";
						elseif (APP_CURR_MINOR != "" && strstr(APP_CURR_MINOR,"RC2")) $kind = "194-RC2";
						elseif (APP_CURR_MINOR != "" && strstr(APP_CURR_MINOR,"beta6")) $kind = "194-beta6";
						elseif (APP_CURR_MINOR != "") $kind = "194-beta";
						else $kind = "194";
					}
					break;
					case "1.93":
					{
						if (APP_CURR_MINOR != "") $kind = "193-beta";
						else $kind = "193";
					}
					break;
					case "1.92":
		...
		elseif ($dbpass == "chatuser_password")
		{
						$kind = "new";
		}
...
if ( $p == 3 )
{
	...
	if ( $_SESSION['dbpass'] != "" ) $C_DB_PASS = $_SESSION['dbpass'];
		else { $C_DB_PASS = C_DB_PASS; if ( $C_DB_PASS == "C_DB_PASS" ) $C_DB_PASS = "chatuser_password"; }
	if ( $_SESSION['dbhost'] != "" ) $C_DB_HOST = $_SESSION['dbhost'];
		else { $C_DB_HOST = C_DB_HOST; if ( $C_DB_HOST == "C_DB_HOST" ) $C_DB_HOST = "localhost"; }
	if ( $_SESSION['dbport'] != "" ) $C_DB_PORT = $_SESSION['dbport'];
		else { $C_DB_PORT = C_DB_PORT; if ( $C_DB_PORT == "C_DB_PORT" ) $C_DB_PORT = ""; }
	if ( $_SESSION['dbtype'] != "" ) $C_DB_TYPE = $_SESSION['dbtype'];
		else { $C_DB_TYPE = C_DB_TYPE; if ( $C_DB_TYPE == "C_DB_TYPE" ) $C_DB_TYPE = $dbtype; }
	if ( $_SESSION['dbdriver'] != "" ) $C_DB_DRIVER = $_SESSION['dbdriver'];
		else { $C_DB_DRIVER = C_DB_DRIVER; if ( $C_DB_DRIVER == "C_DB_DRIVER" ) $C_DB_DRIVER = "mysql"; }
	if ( $_SESSION['dbprefix'] != "" ) $C_DB_PREFIX = $_SESSION['dbprefix'];
		else { $C_DB_PREFIX = C_DB_PREFIX; if ( $C_DB_PREFIX == "C_DB_PREFIX" ) $C_DB_PREFIX = ""; }
...
if ( $p == 4 )
{
	if ( isset($kind) && $kind != "no" && $dbpass != "chatuser_password" && C_DB_PASS != "chatuser_password"){
		if( $dbtype == "mysqli" || $dbtype == "mariadb" ){

			if( $dbtype == "mysqli" ) $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect( $dbhost.($dbport != "" && $dbport != "3306" ? ":".$dbport : ""), $dbuname, $dbpass, "")) OR $error = "Could not connect: ".mysqli_connect_error();
			elseif( $dbtype == "mariadb" ) $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect( $dbhost, $dbuname, $dbpass, "", ($dbport != "" ? $dbport : "3306"))) OR $error = "Could not connect: ".mysqli_connect_error();
			if ( $conn !== false )
			{
				mysqli_query ( $conn, "SET CHARACTER SET utf8");
				mysqli_query ( $conn, "SET NAMES 'utf8'");
				// Get mysql server version and set the corresponding engine type
				$mysql_info = ((is_null($___mysqli_res = mysqli_get_server_info($conn))) ? false : $___mysqli_res);
				$mysql_version = substr($mysql_info, 0, strpos($mysql_info, "-"));
				$engine = (settype($mysql_version, "float") >= "4.1") ? "ENGINE" : "TYPE";
				$timestamp = (settype($mysql_version, "float") >= "5.1") ? "timestamp" : "timestamp(14)";

				$cr_db = mysqli_query( $conn , "CREATE DATABASE IF NOT EXISTS $dbname DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci;") OR "Error creating database: ".mysqli_error($conn);
				if ( $cr_db === false ){
		...
		elseif( $dbtype == "pdo" ){
			...
			$conn = new PDO("$dbdriver:host=$dbhost;port=$dbport;charset=utf8", $dbuname, $dbpass);
			if ( $conn !== false ){
				if( version_compare(PHP_VERSION, '5.3.6', '<') ) $conn->query("SET NAMES 'utf8'");
				// Get MySQL server version and set the corresponding engine type
				$mysql_info = $conn->query('select version()')->fetchColumn();
				$mysql_version = substr($mysql_info, 0, strpos($mysql_info, "-"));
				$engine = (settype($mysql_version, "float") >= "4.1") ? "ENGINE" : "TYPE";
				$timestamp = (settype($mysql_version, "float") >= "5.1") ? "timestamp" : "timestamp(14)";

				$cr_db = $conn->query( $conn , "CREATE DATABASE IF NOT EXISTS $dbname DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci;");
			...
			$conn = mysql_connect ( $dbhost.($dbport != "" && $dbport != "3306" ? ":".$dbport : ""), $dbuname, $dbpass ) OR $error = "Could not connect: ".mysql_error();
			if ( $conn !== false )
			{
				...
				if ( $myconn === false )
				{
					$cr_db = @mysql_create_db ( $dbname, $conn ) OR $error = "Error creating database: ".mysql_error();
#					$cr_db = mysql_query ( "CREATE DATABASE IF NOT EXISTS $dbname DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci;", $conn ) OR $error = "Error creating database: ".mysql_error();
					if ( $cr_db === false )
					{
						$error = sprintf(L_DB_HINT1,$dbname);
					}
	...
	if ( $error == "" )
	{
		...
		if ( $kind == "new" )
		{
			$returned = file_get_contents("database/mysql_new_install.php");
#			settype($returned, "string");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . " UPDATE ".$t_config." SET LANGUAGE = '".$L."'".$LOG_DIR_N." WHERE ID='0';" );
			}
			elseif( $dbtype == "pdo" ){
				$prep_pdo = $conn->prepare ( $returned . " UPDATE ".$t_config." SET LANGUAGE = '".$L."'".$LOG_DIR_N." WHERE ID='0';" );
				$prep_pdo->execute();
			}
			else{
				$queries = preg_split("/;\s*\r\n/", $returned); 
				foreach ( $queries as $query ){ 
				   if(strlen(trim($query)) > 0) mysql_query( $query, $conn ); 
				} 
				mysql_query ( " UPDATE ".$t_config." SET LANGUAGE = '".$L."'".$LOG_DIR_N." WHERE ID='0';", $conn );
			}
		} elseif ( $kind == "195" || $kind == "194-RC5" )
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.95.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
			else{
				$queries = preg_split("/;\s*\r\n/", $returned); 
				foreach ( $queries as $query ){ 
				...
				mysql_query ( $LOG_DIR_NN, $conn );
			}
		} elseif ( $kind == "194-RC4" )
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.94-RC4.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.94-RC3.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.94-RC2.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.94-beta6.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.94-beta.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.93.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.93-beta.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_plus_1.92.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
		...
		{
			$returned = file_get_contents("database/mysql_utf8_convert.php");
			$returned .= '\r\n'.file_get_contents("database/mysql_upgrade_plus_1.90.php");
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
			else{
		...
		{
			$returned = file_get_contents("database/mysql_utf8_convert.php");
			$returned .= '\r\n'.file_get_contents("database/mysql_upgrade_plus_1.8.php");
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
			else{
		...
		{
			$returned = file_get_contents("database/mysql_utf8_convert.php");
			$returned .= '\r\n'.file_get_contents("database/mysql_upgrade_plus_1.7.php");
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
			else{
		...
		{
			$returned = file_get_contents("database/mysql_utf8_convert.php");
			$returned .= '\r\n'.file_get_contents("database/mysql_upgrade_plus_1.0-1.6.php");
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . $LOG_DIR_NN );
			}
			else{
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_std_0.14-0.15.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . " UPDATE ".$t_config." SET LANGUAGE = '".$L."'".$LOG_DIR_N." WHERE ID='0';" );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_std_0.13.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . " UPDATE ".$t_config." SET LANGUAGE = '".$L."'".$LOG_DIR_N." WHERE ID='0';" );
			}
		...
		{
			$returned = file_get_contents("database/mysql_upgrade_std_0.12.php");
			$returned = sprintf($returned, $dbname, $engine, $timestamp, $dbprefix.$t_ban_users, $dbprefix.$t_config, $dbprefix.$t_lurkers, $dbprefix.$t_messages, $dbprefix.$t_reg_users, $dbprefix.$t_stats, $dbprefix.$t_users); // %1$s-%10$s
			if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
				mysqli_multi_query ( $conn, $returned . " UPDATE ".$t_config." SET LANGUAGE = '".$L."'".$LOG_DIR_N." WHERE ID='0';" );
			}
...
if ( $p == 5 )
{
	if ( $kind == "new" || $kind == "014015" || $kind == "013" ||$kind == "012")
	{
		if ( $_POST['admin'] == "" )
		{
			...
			} else
			{
				...
				} else
				{
					...
					if( $dbtype == "mysqli" || $dbtype == "mariadb" ){
						...
						if( $dbtype == "mysqli" ) $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect( $dbhost.($dbport != "" && $dbport != "3306" ? ":".$dbport : ""), $dbuname, $dbpass, $dbname)) OR $error = L_DB_NOCONNECT." ".mysqli_connect_error();
						elseif( $dbtype == "mariadb" ) $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect( $dbhost, $dbuname, $dbpass, $dbname, ($dbport != "" ? $dbport : "3306"))) OR $error = L_DB_NOCONNECT." ".mysqli_connect_error();
						mysqli_query ( $conn, "SET CHARACTER SET utf8");
						mysqli_query ( $conn, "SET NAMES 'utf8'");
						((bool)mysqli_query ( $conn, "USE " . $dbname));

						if ( $kind == "new" ) mysqli_query ( $conn, "INSERT INTO ".$t_reg_users." VALUES ('', '1', '".$admin."', '0', '".$pass_crypt."', '', '', '', '".$chaturl."', '".$adminemail."', 0, 'admin', '', '".time()."', '', 0, 1, '', '', 'http://sourceforge.net/projects/phpmychat', 'http://ciprianmp.com/plus', '', 'red', 'images/avatars/def_avatar.gif', '0', '', '', '', '', '', '', '', '', '', '', '', '');" );
						mysqli_query ( $conn, "UPDATE ".$t_config." SET ADMIN_NAME = '".$adminname."', ADMIN_EMAIL = '".$adminemail."', CHAT_URL = '".$chaturl."', CHAT_NAME = '".$chatname."' WHERE ID='0';" );
					}
					else{
						$conn = mysql_connect ( $dbhost.($dbport != "" && $dbport != "3306" ? ":".$dbport : ""), $dbuname, $dbpass ) OR $error = "Could not connect: ".mysql_error();
						mysql_query("SET CHARACTER SET utf8");
						mysql_query("SET NAMES 'utf8'");
						mysql_select_db ( $dbname ) OR $error = L_DB_NOCONNECT." ".mysql_error();

						if ( $kind == "new" ) mysql_query ( "INSERT INTO ".$t_reg_users." VALUES ('', '1', '".$admin."', '0', '".$pass_crypt."', '', '', '', '".$chaturl."', '".$adminemail."', 0, 'admin', '', '".time()."', '', 0, 1, '', '', 'http://sourceforge.net/projects/phpmychat', 'http://ciprianmp.com/plus', '', 'red', 'images/avatars/def_avatar.gif', '0', '', '', '', '', '', '', '', '', '', '', '', '');", $conn );
						mysql_query ( "UPDATE ".$t_config." SET ADMIN_NAME = '".$adminname."', ADMIN_EMAIL = '".$adminemail."', CHAT_URL = '".$chaturl."', CHAT_NAME = '".$chatname."' WHERE ID='0';", $conn );
					}
				}
			}
		}
...
<form action="install.php?p=<?php echo $p_next."&L=".$L ?>" method="post">
  <tr>
	<td width="100%" style="border-left: 1px solid #111111; border-right-color: #111111; border-right-width: 1">
		...
		<p align="center"><i><b><font size="4" face="Tahoma"><?php echo L_INST_VER." ".APP_VERSION.APP_MINOR ?></font></b></i>
	</td>
  </tr>
  <tr>
...
<?php if ( $p == 1 ) { ?>
  <tr>
	...
	while(list($key, $name) = each($AvailableLanguages))
	{
		...
		elseif ($name == "finnish" && L_ORIG_LANG_FI != "L_ORIG_LANG_FI") $FLAG_NAME = L_ORIG_LANG_FI.($name != $L && L_LANG_FI != "L_LANG_FI" ? "/".L_LANG_FI : "");
...
<?php } ?>
  <tr>
	<td width="100%" align="center" style="border-left-color: #111111; border-left-width: 1; border-right-color: #111111; border-right-width: 1">
		<font face="Tahoma"><b><?php echo L_INST_SETUP." ".sprintf(L_INST_PAG_OF,$p,$p_all) ?></b></font>
	</td>
  </tr>
  <tr>
...
define("C_DB_PORT", '<?php echo $dbport ?>');					// Database listening port (E.g. MySQL default 3306, leave empty; MariaDB - 3307; PostgreSQL - 5432; check your server configuration)
define("C_DB_TYPE", '<?php echo $dbtype ?>');					// SQL server type ("mysqli", "mysql", "mariadb", "pdo", "pgsql" or "odbc")
define("C_DB_DRIVER", '<?php echo $dbdriver ?>');				// Database driver only for PDO ("firebird", "mysql", "odbc", "oci", "pgsql", "sqlite", etc)
define("C_DB_PREFIX", '<?php echo $dbprefix ?>');				// Prefix of your phpmychat tables (default is "c_ ")

// ------ THESE SETTINGS MUST NOT BE CHANGED ------

define("C_BAN_TBL", C_DB_PREFIX.'<?php echo $t_ban_users ?>');	// Name of the table where banished users are stored
define("C_CFG_TBL", C_DB_PREFIX.'<?php echo $t_config ?>');		// Name of the table where configuration settings are stored (if enabled)
define("C_LRK_TBL", C_DB_PREFIX.'<?php echo $t_lurkers ?>');	// Name of the table where data about lurkers are stored (if enabled)
define("C_MSG_TBL", C_DB_PREFIX.'<?php echo $t_messages ?>');	// Name of the table where messages are stored
define("C_REG_TBL", C_DB_PREFIX.'<?php echo $t_reg_users ?>');	// Name of the table where registered users are stored
define("C_STS_TBL", C_DB_PREFIX.'<?php echo $t_stats ?>');		// Name of the table where statistics data is stored (if enabled)
define("C_USR_TBL", C_DB_PREFIX.'<?php echo $t_users ?>');		// Name of the table where user names are stored
...
elseif(C_DB_TYPE == "pdo"){
	$conn = new PDO("".C_DB_DRIVER.":host=".C_DB_HOST.";port=".C_DB_PORT.";dbname=".C_DB_NAME.";charset=utf8", C_DB_USER, C_DB_PASS);
	if(version_compare(PHP_VERSION, '5.3.6', '<')) $conn->query("SET NAMES 'utf8'");
	$query = "SELECT * FROM ".C_CFG_TBL."";
...
else
{
	...
	{
		fputs ( $fh, '// ------ THESE SETTINGS MUST BE MANUALLY COMPLETED (AND NEVER DISCLOSED) ------'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		...
		fputs ( $fh, 'define("C_DB_PORT", \''.$dbport.'\');						// Database listening port (MySQL default 3306, leave empty; MariaDB default 3307 - check your server configuration)'.$lfeed );
		fputs ( $fh, 'define("C_DB_TYPE", \''.$dbtype.'\');					// SQL server type ("mysqli", "mysql", "mariadb", "pgsql" or "odbc")'.$lfeed );
		fputs ( $fh, 'define("C_DB_DRIVER", \''.$dbdriver.'\');					// Database driver only for PDO ("firebird", "mysql", "odbc", "oci", "pgsql", "sqlite", etc)'.$lfeed );
		fputs ( $fh, 'define("C_DB_PREFIX", \''.$dbprefix.'\');						// Prefix of your phpmychat tables (default is "")'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// ------ THESE SETTINGS MUST NOT BE CHANGED ------'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, 'define("C_BAN_TBL", C_DB_PREFIX.\''.$t_ban_users.'\');	// Name of the table where banished users are stored'.$lfeed );
		fputs ( $fh, 'define("C_CFG_TBL", C_DB_PREFIX.\''.$t_config.'\');	// Name of the table where configuration settings are stored (if enabled)'.$lfeed );
		fputs ( $fh, 'define("C_LRK_TBL", C_DB_PREFIX.\''.$t_lurkers.'\');	// Name of the table where data about lurkers are stored (if enabled)'.$lfeed );
		fputs ( $fh, 'define("C_MSG_TBL", C_DB_PREFIX.\''.$t_messages.'\');	// Name of the table where messages are stored'.$lfeed );
		fputs ( $fh, 'define("C_REG_TBL", C_DB_PREFIX.\''.$t_reg_users.'\');	// Name of the table where registered users are stored'.$lfeed );
		fputs ( $fh, 'define("C_STS_TBL", C_DB_PREFIX.\''.$t_stats.'\');		// Name of the table where statistics data is stored (if enabled)'.$lfeed );
		fputs ( $fh, 'define("C_USR_TBL", C_DB_PREFIX.\''.$t_users.'\');		// Name of the table where user names are stored'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, 'if(!isset($ChatPath)) $ChatPath = "";'.$lfeed );
		fputs ( $fh, 'require("${ChatPath}lib/release.lib.php");'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, 'ini_set(\'error_reporting\', E_ALL & ~E_DEPRECATED & ~E_STRICT & ~E_NOTICE & ~E_WARNING);'.$lfeed );
		fputs ( $fh, 'ini_set(\'display_errors\',0); //set to 1 for debugging purposes only'.$lfeed );
		fputs ( $fh, 'ini_set(\'log_errors\',1);'.$lfeed );
		fputs ( $fh, '#mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT); //uncomment for debugging purposes only'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, 'if(C_DB_TYPE == "mysqli" || C_DB_TYPE == "mariadb"){'.$lfeed );
		fputs ( $fh, '	if(C_DB_TYPE == "mysqli") $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect(C_DB_HOST.(C_DB_PORT != "" && C_DB_PORT != "3306" ? ":".C_DB_PORT : ""), C_DB_USER, C_DB_PASS, C_DB_NAME)) or die (\'<center>Error: Could Not Connect To Database: \'.mysqli_connect_error());'.$lfeed );
		fputs ( $fh, '	elseif(C_DB_TYPE == "mariadb") $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect(C_DB_HOST, C_DB_USER, C_DB_PASS, C_DB_NAME, (C_DB_PORT != "" ? C_DB_PORT : "3306"))) or die (\'<center>Error: Could Not Connect To Database: \'.mysqli_connect_error());'.$lfeed );
		fputs ( $fh, '	mysqli_query($conn, "SET CHARACTER SET utf8");'.$lfeed );
		fputs ( $fh, '	mysqli_query($conn, "SET NAMES \'utf8\'");'.$lfeed );
		fputs ( $fh, '#	((bool)mysqli_query($conn, "USE " . constant(\'C_DB_NAME\')));'.$lfeed );
		fputs ( $fh, '	$query = "SELECT * FROM ".C_CFG_TBL."";'.$lfeed );
		fputs ( $fh, '	$result = mysqli_query($conn, $query);'.$lfeed );
		fputs ( $fh, '	$row = mysqli_fetch_assoc($result);'.$lfeed );
		fputs ( $fh, '}'.$lfeed );
		fputs ( $fh, 'elseif(C_DB_TYPE == "pdo"){'.$lfeed );
		fputs ( $fh, '	$conn = new PDO("".C_DB_DRIVER.":host=".C_DB_HOST.";port=".C_DB_PORT.";dbname=".C_DB_NAME.";charset=utf8", C_DB_USER, C_DB_PASS);'.$lfeed );
		fputs ( $fh, '	if( version_compare(PHP_VERSION, \'5.3.6\', \'<\') ) $conn->query("SET NAMES \'utf8\'");'.$lfeed );
		fputs ( $fh, '	$query = "SELECT * FROM ".C_CFG_TBL."";'.$lfeed );
		fputs ( $fh, '	$result = $conn->query($query);'.$lfeed );
		fputs ( $fh, '	$row = $result->fetch(PDO::FETCH_ASSOC);'.$lfeed );
		fputs ( $fh, '}'.$lfeed );
		fputs ( $fh, 'else{'.$lfeed );
		fputs ( $fh, '	$conn = mysql_connect(C_DB_HOST.(C_DB_PORT != "" && C_DB_PORT != "3306" ? ":".C_DB_PORT : ""), C_DB_USER, C_DB_PASS) or die (\'<center>Error: Could Not Connect: " \'.mysql_error());'.$lfeed );
		fputs ( $fh, '	mysql_query("SET CHARACTER SET utf8");'.$lfeed );
		fputs ( $fh, '	mysql_query("SET NAMES \'utf8\'");'.$lfeed );
		fputs ( $fh, '	mysql_select_db(C_DB_NAME) or die (\'<center>Error: Could Not Connect To Database: \'.mysql_error());'.$lfeed );
		fputs ( $fh, '	$query = "SELECT * FROM ".C_CFG_TBL."";'.$lfeed );
		fputs ( $fh, '	$result = mysql_query($query);'.$lfeed );
		fputs ( $fh, '	$row = mysql_fetch_assoc($result);'.$lfeed );
		fputs ( $fh, '}'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, 'while(list($name,$value) = each($row))'.$lfeed );
		fputs ( $fh, '{'.$lfeed );
		fputs ( $fh, '	$$name = $value;'.$lfeed );
		fputs ( $fh, '};'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		...
		fputs ( $fh, 'if(C_DB_TYPE == "mysqli" || C_DB_TYPE == "mariadb"){'.$lfeed );
		fputs ( $fh, '	$result_bot = mysqli_query($conn, $query_bot);'.$lfeed );
		fputs ( $fh, '	list($BOT_NAME, $BOT_AVATAR, $BOT_FONT_COLOR) = mysqli_fetch_row($result_bot);'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '	$result_quote = mysqli_query($conn, $query_quote);'.$lfeed );
		fputs ( $fh, '	list($QUOTE_NAME, $QUOTE_AVATAR, $QUOTE_FONT_COLOR) = mysqli_fetch_row($result_quote);'.$lfeed );
		fputs ( $fh, '	@((is_null($___mysqli_res = mysqli_close($conn))) ? false : $___mysqli_res);'.$lfeed );
		fputs ( $fh, '}'.$lfeed );
		fputs ( $fh, 'elseif(C_DB_TYPE == "pdo"){'.$lfeed );
		fputs ( $fh, '	$result_bot = $conn->query($query_bot);'.$lfeed );
		fputs ( $fh, '	list($BOT_NAME, $BOT_AVATAR, $BOT_FONT_COLOR) = $result_bot->fetch(PDO::FETCH_ASSOC);'.$lfeed );
		fputs ( $fh, '	$result_bot->closeCursor();'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '	$result_quote = $conn->query($query_quote);'.$lfeed );
		fputs ( $fh, '	list($QUOTE_NAME, $QUOTE_AVATAR, $QUOTE_FONT_COLOR) = $result_quote->fetch(PDO::FETCH_ASSOC);'.$lfeed );
		fputs ( $fh, '	$result_quote->closeCursor();'.$lfeed );
		fputs ( $fh, '	$conn = null;'.$lfeed );
		fputs ( $fh, '}'.$lfeed );
		fputs ( $fh, 'else{'.$lfeed );
		...
		fputs ( $fh, '	}'.$lfeed );
		fputs ( $fh, '	if($WELCOME_SOUND != "")'.$lfeed );
		fputs ( $fh, '	{'.$lfeed );
		...
		fputs ( $fh, 'if ($ALLOW_BUZZ_SOUND && $BUZZ_SOUND != "") define("L_BUZZ_SND", "<EMBED SRC=\"".$BUZZ_SOUND."\" VOLUME=\"30\" HIDDEN=\"true\" AUTOSTART=\"true\" LOOP=\"false\" NAME=\"Buzz\" MASTERSOUND><NOEMBED><BGSOUND SRC=\"".$BUZZ_SOUND."\" LOOP=1></NOEMBED></EMBED>");'.$lfeed );
		...
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// ------ THIS IS THE LIST OF COLORS DISPLAYED IN YOUR DROP-DOWN LIST. IF YOU KNOW OR WANT MORE, JUST ADD THEM HERE ------'.$lfeed );
		...
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// Classic short list - comment the above line and uncomment below to use the standard version list (also customizable)'.$lfeed );
		fputs ( $fh, '#define("COLORLIST", \'"","Black","Red","Green","Blue","Purple","Darkred","Darkgreen","Darkblue","Maroon","Aqua","Darkorange"\');'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '/* backup colorlist (to revert to default)'.$lfeed );
		fputs ( $fh, 'define("COLORLIST", \'"","black","dimgray","gray","darkgray","silver","lightgrey","gainsboro","whitesmoke","ghostwhite","white","slategray","lightslategray","midnightblue","navy","darkblue","darkslateblue","mediumblue","blue","steelblue","royalblue","cornflowerblue","dodgerblue","deepskyblue","lightskyblue","skyblue","lightsteelblue","lightblue","powderblue","paleturquoise","lightcyan","aliceblue","azure","mintcream","darkslategray","cadetblue","teal","darkcyan","lightseagreen","darkturquoise","mediumturquoise","turquoise","aqua","cyan","mediumaquamarine","aquamarine","darkolivegreen","olive","olivedrab","darkkhaki","darkgreen","green","forestgreen","seagreen","mediumseagreen","darkseagreen","mediumspringgreen","springgreen","palegreen","honeydew","limegreen","lime","lightgreen","lawngreen","chartreuse","greenyellow","yellowgreen","indigo","purple","darkmagenta","darkviolet","darkorchid","mediumorchid","orchid","violet","plum","thistle","blueviolet","mediumpurple","slateblue","mediumslateblue","lavender","mediumvioletred","magenta","fuchsia","deeppink","palevioletred","hotpink","lightpink","pink","mistyrose","lavenderblush","maroon","darkred","firebrick","crimson","red","orangered","tomato","indianred","lightcoral","salmon","darksalmon","lightsalmon","coral","darkorange","orange","sandybrown","darkgoldenrod","goldenrod","gold","yellow","khaki","palegoldenrod","lemonchiffon","cornsilk","lightgoldenrodyellow","beige","lightyellow","ivory","rosybrown","saddlebrown","brown","sienna","chocolate","peru","tan","burlywood","wheat","navajowhite","peachpuff","moccasin","bisque","blanchedalmond","papayawhip","antiquewhite","linen","oldlace","seashell","floralwhite","snow"\');'.$lfeed );
		fputs ( $fh, '*/'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		...
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// Sound Notification mod by Ciprian'.$lfeed );
		fputs ( $fh, 'define("C_NOTIFY_SOUND", $NOTIFY_SOUND);'.$lfeed );
		fputs ( $fh, '?>' );
...
else
{
	...
	if ( ! fputs ( $fhc, '<?php'.$lfeed ) )
	{
		$error .= $lfeed.'Could not write into "config/curr_version.lib.php"!';
	}	// END OF COULD NOT WRITE
	else
	{
		fputs ( $fhc, '// Do NOT remove this file!'.$lfeed );
		fputs ( $fhc, '// App current version values for future upgrade purposes.'.$lfeed );
		fputs ( $fhc, '// This file had been created/updated by the installer.'.$lfeed );
		fputs ( $fhc, 'define("APP_CURR_VERSION", \''.APP_VERSION.'\');'.$lfeed );
		fputs ( $fhc, 'define("APP_CURR_MINOR", \''.APP_MINOR.'\');	// E.g. "-ß1"'.$lfeed );
		fputs ( $fhc, '?>' );
	}	// END OF WRITE INTO config/curr_version.lib.php
...
<?php if ( $p == 1 && $do_ftp) { ?>
	...
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P0_FORM1 ?></font></td>
		<td><input type="text" name="ftphost" size="20" value="<?php echo $ftphost ?>"></p></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P0_FORM2 ?></font></td>
		<td><input type="text" name="ftpuname" size="20" value="<?php echo $ftpuname ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P0_FORM3 ?></font></td>
		<td><input type="password" name="ftppass" size="20" value="<?php echo $ftppass ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2">&nbsp;</font></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P0_FORM4 ?></font></td>
		<td><input type="text" name="ftppath" size="20" value="<?php echo $ftppath ?>"></td>
	</tr>
	</table>
<?php } ?>
<?php if ( $p == 2 ) { ?>
<?php if ( $error == "" && $error3 == "" ) { ?>
	<p align="center">
		...
		<option value="195"<?php if ($kind=="195") echo " selected" ?>><?php echo sprintf(L_P1_OP02,"phpMyChat-Plus 1.95") ?></option>
		<option value="194-RC5"<?php if ($kind=="194-RC5") echo " selected" ?>><?php echo sprintf(L_P1_OP02,"phpMyChat-Plus 1.94-RC5") ?></option>
...
<?php } elseif ( $p == 3 ) { ?>
	...
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM01 ?></font></td>
		<td><input type="text" name="dbhost" size="20" value="<?php echo $C_DB_HOST ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM17 ?></font></td>
		<td><input type="text" name="dbport" size="1" value="<?php echo $C_DB_PORT ?>"></p></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM02 ?></font></td>
		<td><input type="text" name="dbuname" size="20" value="<?php echo $C_DB_USER ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM03 ?></font></td>
		<td><input type="password" name="dbpass" size="20" value="<?php echo $C_DB_PASS ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2">&nbsp;</font></td>
		<td>&nbsp;</td>
	...
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM04 ?></font></td>
		<td><input type="text" name="dbname" size="20" value="<?php echo $C_DB_NAME ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2">&nbsp;</font></td>
		<td>&nbsp;</td>
	...
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM05 ?></font></td>
		<td>
			<select size="1" name="dbtype">
			<option value="mysqli"<?php if ( $C_DB_TYPE == "mysqli" ) echo " SELECTED" ?>>MySQLi</option>
			<option value="mysql"<?php if ( $C_DB_TYPE == "mysql" ) echo " SELECTED" ?>>MySQL</option>
			<option value="mariadb"<?php if ( $C_DB_TYPE == "mariadb" ) echo " SELECTED" ?>>MariaDB</option>
			<option value="odbc"<?php if ( $C_DB_TYPE == "odbc" ) echo " SELECTED" ?>>ODBC (not available)</option>
			<option value="pdo"<?php if ( $C_DB_TYPE == "pdo" ) echo " SELECTED" ?>>PDO</option>
			<option value="pgsql"<?php if ( $C_DB_TYPE == "pgsql" ) echo " SELECTED" ?>>Postgree SQL (not available)</option>
			</select>
		</td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM18 ?></font></td>
		<td>
			<?php
				$PDODrivers = PDO::getAvailableDrivers();
			?>
			<select size="1" name="dbdriver">
				<option value=""<?php if ( $C_DB_DRIVER == "" ) echo " SELECTED"; if( empty($PDODrivers) ) echo " DISABLED"; ?>>FireBird</option>
				<?php
				if (in_array("firebird",$PDODrivers,TRUE)) echo("<option value=\"firebird\"".( $C_DB_DRIVER == "firebird" ? " SELECTED" : "").">FireBird</option>");
				if (in_array("mysql",$PDODrivers,TRUE)) echo("<option value=\"mysql\"".( $C_DB_DRIVER == "mysql" ? " SELECTED" : "").">MySQL</option>");
				if (in_array("oci",$PDODrivers,TRUE)) echo("<option value=\"oci\"".( $C_DB_DRIVER == "oci" ? " SELECTED" : "").">OCI</option>");
				if (in_array("odbc",$PDODrivers,TRUE)) echo("<option value=\"odbc\"".( $C_DB_DRIVER == "odbc" ? " SELECTED" : "").">ODBC</option>");
				if (in_array("pgsql",$PDODrivers,TRUE)) echo("<option value=\"pgsql\"".( $C_DB_DRIVER == "pgsql" ? " SELECTED" : "").">Postgree SQL</option>");
				if (in_array("sqlite",$PDODrivers,TRUE)) echo("<option value=\"sqlite\"".( $C_DB_DRIVER == "sqlite" ? " SELECTED" : "").">SQLite</option>");
				?>
			</select>
		</td>
	</tr>
	...
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM19 ?></font></td>
		<td><input type="text" name="dbprefix" size="20" value="<?php echo $C_DB_PREFIX ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM09 ?></font></td>
		<td><input type="text" name="t_ban_users" size="20" value="<?php echo preg_replace("/^".C_DB_PREFIX."/", "", $C_BAN_TBL); ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM10 ?></font></td>
		<td><input type="text" name="t_config" size="20" value="<?php echo preg_replace('/^'.C_DB_PREFIX.'/', "", $C_CFG_TBL); ?>"> <font color="#FF0000">*</font></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM11 ?></font></td>
		<td><input type="text" name="t_lurkers" size="20" value="<?php echo preg_replace('/^'.C_DB_PREFIX.'/', "", $C_LRK_TBL); ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM06 ?></font></td>
		<td><input type="text" name="t_messages" size="20" value="<?php echo preg_replace('/^'.C_DB_PREFIX.'/', "", $C_MSG_TBL) ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM08 ?></font></td>
		<td><input type="text" name="t_reg_users" size="20" value="<?php echo preg_replace('/^'.C_DB_PREFIX.'/', "", $C_REG_TBL); ?>"> <font color="#FF0000">*</font></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM16 ?></font></td>
		<td><input type="text" name="t_stats" size="20" value="<?php echo preg_replace('/^'.C_DB_PREFIX.'/', "", $C_STS_TBL); ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM07 ?></font></td>
		<td><input type="text" name="t_users" size="20" value="<?php echo preg_replace('/^'.C_DB_PREFIX.'/', "", $C_USR_TBL); ?>"></td>
	</tr>
	<tr>
		<td width="260"><font face="Tahoma" size="2">&nbsp;</font></td>
		<td>&nbsp;</td>
	...
	<tr>
		<td width="260"><font face="Tahoma" size="2"><?php echo L_P2_FORM12 ?></font></td>
		<td>
	...
	<tr>
		...
		<td><input type="text" name="chatname" size="25" value="<?php echo $chatname ?>"></td>
	</tr>
	<tr>
		<td width="260">&nbsp;</td>
		<td>&nbsp;</td>
	...
	<tr>
		<td width="398" colspan="2">
			...
			<br /><font color="#FF0000">**</font> <?php echo L_P2_FORM14 ?></font></i>
		</td>
	</tr>
	</table>
<?php } elseif ( $p == 4 ) { ?>
...
<?php if ( ( $error == "" ) && ( $kind == "new" ) ) { ?>
	<tr>
		...
		<td><input type="text" name="admin" size="25" value="<?php echo $admin ?>"></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
	...
	<tr>
		...
		<td><input type="password" name="pass1" size="25" value="<?php echo $pass1 ?>"></td>
	</tr>
	<tr>
		<td><font size="2" face="Tahoma"><?php echo L_P3_FORM3 ?></font></td>
		<td><input type="password" name="pass2" size="25" value="<?php echo $pass2 ?>"></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
	...
	<tr>
		...
		<td><input type="text" name="adminname" size="25" value="<?php echo $adminname ?>"></td>
	</tr>
	<tr>
		<td><font face="Tahoma" size="2"><?php echo L_P3_FORM5 ?></font></td>
		<td><input type="text" name="adminemail" size="25" value="<?php echo $adminemail ?>"></td>
	</tr>
	<tr>
		<td><font face="Tahoma" size="2"><?php echo L_P3_FORM6 ?></font></td>
		<td><input type="text" name="chaturl" size="25" value="<?php echo $chaturl ?>"></td>
	</tr>
<?php
...
<?php if ( $error4 == "" ) { ?>
	...
	<tr>
		<td bgcolor="#FFFFFF"><p align="justify"><b><font face="Tahoma" size="2" color="#FF0000"><?php echo L_P4_HINT4 ?></font></b></td>
	</tr>
	<tr>
		<td width="100%" style="border-left: medium none #111111; border-right: medium none #111111">&nbsp;</td>
	</tr>
	<tr>
		<td><p align="justify"><b><font face="Tahoma" size="2" color="#0000FF"><?php echo L_P4_HINT3 ?> </font></b>
<?php }
} ?>
<?php if ( ( $error == "" ) && ( $error4 == "" ) ) {
