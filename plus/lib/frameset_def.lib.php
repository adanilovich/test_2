...
{
	?>
	<FRAMESET COLS="100%,*,*" FRAMEBORDER="0" BORDER="0" FRAMESPACING="0" OnResize="if (document.layers) window.location = '<?php echo("$From?L=$L&Ver=$Ver&U=$U1$AddPwd2Url&R=$R1&T=$T&D=$D&N=$N&Reload=NNResize"); ?>';" allowfullscreen="allowfullscreen">
...
if ((isset($dropdownmsga) && ($status == "a" || $status == "t")) || (isset($dropdownmsgm) && $status == "m") || isset($dropdownmsg))
{
	...
	{
?>
			<FRAMESET ROWS="40,*,60" FRAMEBORDER="0" BORDER="0" FRAMESPACING="0">
<?php
	}
	else
	{
...
?>
			...
			<FRAMESET ROWS="80,*<?php echo($player.$support); ?>,55" FRAMEBORDER="0" BORDER="0" FRAMESPACING="0">
				...
				{
				?>
					<FRAME SRC="<?php echo($ChatPath); ?>player.php" ID="streaming" NAME="streaming" FRAMESPACING="0" MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING="NO">
				<?php
				}
				?>
...
if(C_USE_FLAGS && ($status == "a" || $status == "t" || $status == "m" || C_SHOW_FLAGS))
	{
	?>
		<FRAMESET COLS="*,220" FRAMEBORDER="0" BORDER="0" FRAMESPACING="0" OnResize="if (document.layers) window.location = '<?php echo("$From?L=$L&Ver=$Ver&U=$U1$AddPwd2Url&R=$R1&T=$T&D=$D&N=$N&Reload=NNResize"); ?>';" allowfullscreen="allowfullscreen">
	<?php
	}
	else
	{
	?>
		<FRAMESET COLS="*,190" FRAMEBORDER="0" BORDER="0" FRAMESPACING="0" OnResize="if (document.layers) window.location = '<?php echo("$From?L=$L&Ver=$Ver&U=$U1$AddPwd2Url&R=$R1&T=$T&D=$D&N=$N&Reload=NNResize"); ?>';" allowfullscreen="allowfullscreen">
	<?php
	}
if ((isset($dropdownmsga) && ($status == "a" || $status == "t")) || (isset($dropdownmsgm) && $status == "m") || isset($dropdownmsg))
{
...
?>
      ...
      <FRAME SRC="<?php echo($ChatPath); ?>topic.php?<?php echo("L=$L&R=$R1"); ?>" NAME="topic" FRAMEBORDER="0" BORDER="0" FRAMESPACING="0" MARGINWIDTH="0" MARGINHEIGHT="0" SCROLLING="NO">
		...
		?>
		<FRAMESET ROWS="80,*<?php echo($player.$support)?>,60" BORDER=0 allowfullscreen="allowfullscreen">
			...
			{
			?>
				<FRAME SRC="<?php echo($ChatPath); ?>player.php" NAME="streaming" MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING="NO" allowfullscreen="allowfullscreen">
			<?php
			}
			?>
