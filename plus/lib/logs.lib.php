...
if ($i > 1)
{
	...
	if ($message_nb > 0)
	{
		...
		{
			if (file_exists("./".C_LOG_DIR."/".$year."/".$month."") && substr(decoct(fileperms("./".C_LOG_DIR."/".$year."/".$month."")),1) != "0777")
			{
					chmod("./".C_LOG_DIR."/".$year."/".$month, 0777);
			}
	...
	#		@fwrite($fp, sprintf('$longdtformat = ($L == "english" ? str_replace("%d of", ((stristr(PHP_OS,"win") ? "%#d" : "%e")."<sup>".date("S",$CorrectedTime)."</sup> of"), L_LONG_DATETIME) : L_LONG_DATETIME);'.$eol));
			...
			@fwrite($fp, sprintf("$eol<!-- MESSAGES TABLE STARTS BELOW -->$eol$eol<table border=1 cellspacing=0 cellpading=1>$eol<tr style=\"font-weight:bold; color:blue; background-color=yellow\" align=\"center\">$eol<td nowrap=\"nowrap\"><?php echo (A_POST_TIME); ?></td>$eol<td nowrap=\"nowrap\"><?php echo (A_CHTEX_ROOM); ?></td>$eol<td nowrap=\"nowrap\"><?php echo (A_FROM_TO); ?></td>$eol<td style=\"width:98%%\"><?php echo (A_CHTEX_MSG); ?></td>$eol</tr>"));
		}
		else
		{
			$fp = fopen($logpath, "a") ;
		...
		flock($fp, LOCK_UN);
		fclose($fp) ;
		chmod("./".C_LOG_DIR."/".$year."/".$month, 0755);
		$done = 1;
	}
	$i = 0;
}
...
if ($iu > 1)
{
	...
	if ($message_nbu > 0)
	{
		...
		{
			if (file_exists("./logs/".$yearu."/".$monthu."") && substr(decoct(fileperms("./logs/".$yearu."/".$monthu."")),1) != "0777")
			{
					chmod("./logs/".$yearu."/".$monthu, 0777);
			}
	...
	#		@fwrite($fpu, sprintf('$longdtformat = ($L == "english" ? str_replace("%d of", ((stristr(PHP_OS,"win") ? "%#d" : "%e")."<sup>".date("S",$CorrectedTime)."</sup> of"), L_LONG_DATETIME) : L_LONG_DATETIME);'.$eol));
			...
			@fwrite($fpu, sprintf($eol."<!-- MESSAGES TABLE STARTS BELOW -->$eol$eol<table border=1 cellspacing=0 cellpading=1>$eol<tr style=\"font-weight:bold; color:blue; background-color=yellow\" align=\"center\">$eol<td nowrap=\"nowrap\"><?php echo (A_POST_TIME); ?></td>$eol<td nowrap=\"nowrap\"><?php echo (A_CHTEX_ROOM); ?></td>$eol<td nowrap=\"nowrap\"><?php echo (A_FROM); ?></td>$eol<td style=\"width:98%%\"><?php echo (A_CHTEX_MSG); ?></td>$eol</tr>"));
		}
		else
		{
			$fpu = fopen($logpathu, "a") ;
